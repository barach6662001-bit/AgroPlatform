...
export async function getApiKeys(): Promise<ApiKeyDto[]> {
  const response = await axios.get<ApiKeyDto[]>('/api/api-keys');
...
export async function createApiKey(request: CreateApiKeyRequest): Promise<CreateApiKeyResponse> {
  const response = await axios.post<CreateApiKeyResponse>('/api/api-keys', request);
  return response.data;
}
...
export async function revokeApiKey(apiKeyId: string): Promise<void> {
  await axios.post(`/api/api-keys/${apiKeyId}/revoke`);
}
