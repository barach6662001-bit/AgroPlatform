...
export function usePublicDemoAutoLogin(): { ready: boolean; failed: boolean } {
  ...
  useEffect(() => {
    ...
    // If the visitor explicitly lands on "/", /login or /landing, let them
    // see those pages instead of silently auto-logging them in as demo. The
    // landing page exposes a "Спробувати демо" CTA that navigates into the
    // app, at which point this hook re-runs and logs the visitor in.
    if (typeof window !== 'undefined') {
      const p = window.location.pathname;
      if (p === '/' || p === '/login' || p === '/landing') { setReady(true); return; }
    }
