...
export default function GrainStorageList() {
  ...
  const load = (p = page, ps = pageSize) => {
    setLoading(true);
    getWarehouses({ page: p, pageSize: ps, type: 1 })
      .then(setResult)
  ...
  const handleCreate = async () => {
    try {
      ...
      setSaving(true);
      await createWarehouse({ ...values, type: 1 });
  ...
  const columns = [
    {
      ...
      dataIndex: 'name',
      key: 'name',
      sorter: (a: WarehouseDto, b: WarehouseDto) => a.name.localeCompare(b.name),
    },
    {
    ...
    },
    {
      ...
      render: (_: unknown, record: WarehouseDto) => (
        <a onClick={() => navigate(`/warehouses/items?warehouse=${record.id}`)}>
          {t.warehouses.balances}
        </a>
      ),
    },
  ];
  ...
  return (
    <div>
      <PageHeader title={t.warehouses.grainStorageTitle} subtitle={t.warehouses.grainStorageSubtitle} />
      <Space style={{ marginBottom: 16 }}>
        {canCreate && (
          ...
          >
            {t.warehouses.createGrainStorage}
          </Button>
        )}
      </Space>
      {result === null ? (
      ...
      <Modal
        title={t.warehouses.createGrainStorage}
        open={modalOpen}
        onOk={handleCreate}
