...
public class ReceiptStockHandlerTests
{
    ...
    public async Task Handle_ValidReceipt_CreatesStockMoveAndBalance()
    {
        ...
        var handler = new ReceiptStockHandler(context, dateTime);
        ...
        var move = await ((TestDbContext)context).StockMoves.FindAsync(moveId);
        move.Should().NotBeNull();
        move!.Quantity.Should().Be(100m);

        var balance = await ((TestDbContext)context).StockBalances
