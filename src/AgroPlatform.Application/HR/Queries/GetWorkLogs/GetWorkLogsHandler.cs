...
public class GetWorkLogsHandler : IRequestHandler<GetWorkLogsQuery, IReadOnlyList<WorkLogDto>>
{
    ...
    public async Task<IReadOnlyList<WorkLogDto>> Handle(GetWorkLogsQuery request, CancellationToken cancellationToken)
    {
        ...
        return await query
            .OrderBy(w => w.WorkDate).ThenBy(w => w.Employee.LastName)
